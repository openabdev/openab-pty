...
impl Session {
    /// Write runtime-generated bytes (capability-query answers) to the child.
    /// Unlike `write_input` this carries no client connection and is not fenced
    /// on the owner: the runtime is answering on the terminal's own behalf, and
...
impl SessionManager {
    ...
    fn on_output(&self, session: &Arc<Session>, bytes: &[u8]) {
        // Answer static capability queries at the source and remove them from
        // the stream before anything else sees them (#15): they must not reach
        // the ring buffer (or a `?since=` replay re-delivers them) nor a client
        // (which would answer a query the runtime already answered). The reply
        // goes straight back to the child, so a detached session negotiates
        // with no client attached. CPR is not proxied and passes through.
        let forwarded: std::borrow::Cow<[u8]> = match &session.proxy {
            Some(proxy) => {
                let (stripped, response) = proxy.lock().process(bytes);
                if !response.is_empty() {
                    session.answer_child(&response);
                }
                stripped
            }
        ...
        session.io.buffer.lock().write(&forwarded);
        session.io.touch(self.epoch);
        let subscriber = session.io.subscriber.lock().clone();
        if let Some(outbound) = subscriber {
            outbound.push_bytes(&forwarded);
        }
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn a_capability_query_from_the_child_is_answered_and_stripped() {
        // #15: the runtime answers static queries itself. The DA query must be
        // written back to the child and must NOT reach the client or the ring.
        let spawner = FakeSpawner::new();
        let manager = manager_with(spawner.clone(), "", SessionPolicy::default());
    ...
    #[tokio::test]
    async fn a_detached_session_still_answers_capability_queries() {
        // A TUI that gates on DA (Devin) must negotiate with no client attached.
        let spawner = FakeSpawner::new();
        let manager = manager_with(spawner.clone(), "", SessionPolicy::default());
        let _created = manager.create(name("solo"), WindowSize::default()).unwrap();

        spawner.emit(b"\x1b[c");
        tokio::time::sleep(Duration::from_millis(50)).await;
